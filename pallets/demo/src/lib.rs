...
pub use super::*;
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn update_age(origin: OriginFor<T>, id: Id, new_age: u8) -> DispatchResult {
			let who = ensure_signed(origin)?;
			let student = <Students<T>>::get(id).unwrap();
			ensure!(student.account == who, Error::<T>::StorageOverflow);
			...
			Students::<T>::insert(id.clone(), new_student);
			Ok(())
			
		}
		#[pallet::weight(10_000 + T::DbWeight::get().reads(1))]
		pub fn update_name(origin: OriginFor<T>, id: Id, new_name: Vec<u8>) -> DispatchResult {
			let who = ensure_signed(origin)?;
			let student = <Students<T>>::get(id).unwrap();
			ensure!(student.account == who, Error::<T>::StorageOverflow);
			let new_student = Student {
			...
			Students::<T>::insert(id, new_student);
			Ok(())
		}
	}
}
